...
_shared = Path(__file__).resolve().parent.parent / "shared" / "protocol.py"
spec = importlib.util.spec_from_file_location("clients_shared_protocol", _shared)
if spec is None or spec.loader is None:
    raise ImportError(f"cannot load protocol from {_shared}")
mod = importlib.util.module_from_spec(spec)
sys.modules["clients_shared_protocol"] = mod
