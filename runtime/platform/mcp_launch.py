...
_LAUNCH_META = frozenset(
    {"enabled", "label", "builtin", "has_env", "has_headers", "sidecar", "sidecar_id"}
)
...
def launch_servers(path: Path | None = None) -> dict[str, dict]:
    ...
    HTTP servers only expose ``type`` / ``url`` / ``headers`` so Runtime-only
    fields like spawn ``env`` (e.g. Playwright extension token) stay off-disk
    for the agent cwd ``.mcp.json``.
    """
    ...
    for name, entry in read_servers(source).items():
        ...
        if cleaned.get("url") or str(cleaned.get("type") or "").lower() == "http":
            row: dict = {}
            if cleaned.get("type"):
